...
void Graph::initialize()  // this function is for source and sink
{
	...
	{
		addEdge(this->source, graphNode.at(i), rand() % 10 + 5, 0);  // from source to sparce graph
		addEdge(graphNode.at(i), this->sink, rand() % 10 + 5, 0);  // from sparce graph to destination
...
void Graph::setNodes()
{
	// the outflow of source is the inflow of dest
